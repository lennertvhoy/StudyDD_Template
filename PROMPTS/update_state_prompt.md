...
3. Add SkillSignal packets to `sessions/SKILLSIGNAL_PACKETS.md` when useful.
4. Update `state/SKILL_MAP.yaml`:
...
5. Update `state/STUDY_STATE.yaml`:
   - current_readiness_estimate
   - readiness_confidence
   - active_focus
   - blocking_confusions
   - session_history
6. Update `reviews/REVIEW_QUEUE.md` for weak, repaired, or shaky answers.
7. Update `NEXT_ACTIONS.md` with the immediate next step.
8. Update `state/STUDY_STATUS.md` with a short human-readable snapshot.
9. Run `python3 scripts/check_studydd.py`.
