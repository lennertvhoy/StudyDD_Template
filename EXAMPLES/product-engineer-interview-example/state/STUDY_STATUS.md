# STUDY_STATUS — Product Engineer Interview Example

> Example state for a learner preparing for a product engineer interview.

## Learner

- **Name:** Sam
- **Target:** Product engineer role at a mid-size SaaS company
- **Current context:** Interview loop in three weeks; system design and behavioral are priorities
- **Preferred language:** English
- **Preferred tutoring style:** Mock interviews with immediate feedback

## Study target

- **Title:** Product Engineer Interview Loop
- **Exam or goal:** Product engineer onsite / virtual loop
- **Deadline:** 2026-07-15
- **Current readiness estimate:** 45%
- **Readiness confidence:** Medium
- **Risk level:** Moderate — weak on system design tradeoffs and behavioral metrics

## Active focus

- **Current topic:** System design tradeoffs for product features
- **Reason:** Struggles to explain why one design wins over another in business terms
- **Next question:** Q-20260624-PE-003 — design a notification system for a SaaS product and justify the tradeoffs
- **Blocking confusions:** When to prioritize consistency over availability in a product feature

## Quick summary

- Confirmed strengths: 2
- Weak areas: 3
- Pending validation: 2
- Last session: 2026-06-23 — behavioral mock interview
- Next action: Complete Q-20260624-PE-003 system design question, then rehearse a metrics-driven behavioral answer.
